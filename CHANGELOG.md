...
- 新增可管理的对话字体系统：管理员可在后台维护字体，用户可在个人资料页选择聊天正文显示字体，聊天页会按用户偏好应用字体。
- 新增多款代理加载的中文表现字体：马善政、龙藏、站酷庆科黄油、站酷小薇、志莽行等，并支持个人资料页真实字体预览。
- 新增用户侧会话列表与聊天页会话操作的居中弹窗交互，消息操作、会话管理“更多”和聊天页顶部“更多”保持一致体验。

### Changed

- 静态资源版本号改为集中管理：模板通过 `assetUrl()` / 文件指纹自动生成资源版本，减少手写 `?v=` 带来的漏改和缓存问题。
- 字体加载改为按需加载：全站基础字体仍统一加载，远程中文字体只在聊天页或个人资料字体预览等需要的页面加载，避免无差别请求拖慢页面。
- 全站按钮系统进一步统一，移除残留的紫色/蓝色实心主按钮，调整为更克制的中性深色描边风格。
- 优化聊天富文本渲染与阅读样式，折叠块占位符不再外露，正文强调、引用、代码与列表的视觉层级更稳定。

### Fixed

- 修复共享字体代理返回大体积 `.ttf` 和多字体批量加载容易触发 503 的问题，改为代理域名、浏览器 UA 与 woff2 样式表请求策略。
- 修复聊天消息操作按钮在用户消息中不可见、点击热区偏小，以及操作菜单受消息气泡布局限制的问题。
- 修复会话操作菜单内联展开导致的移动端与布局约束问题，改为页面级居中弹窗。

## [1.5.0] - 2026-05-08

### Added

- 用户控制台的最近会话区域调整为仅展示最近 3 个会话，并提供“会话管理”入口。
- 新增用户侧会话管理页 `/conversations`，支持按关键词、角色、状态筛选全部会话，支持分页、排序和批量归档/恢复/删除。

### Changed

- 全站新增会话管理导航入口，并将 CSS/JS 静态资源引用统一追加 `v1.5.0-20260508` 版本参数，避免浏览器继续命中旧缓存。
...
- 全站核心文案升级为生产项目口吻，覆盖首页、登录、控制台、公开角色、聊天、角色编辑与管理后台关键页面。
- 统一用户可见术语：套餐改为权益/权益方案，Token 额度改为文字额度，普通用户流程中的 Prompt/system prompt 改为规则或角色配置说明。
- 优化后台管理文案，补充用户权限、权益切换、模型服务、规则配置、会话记录、日志和通知配置的操作边界与风险提示。
- 同步补齐简体中文、繁体中文与英文 i18n 词典，确保新增文案通过完整性检查。
